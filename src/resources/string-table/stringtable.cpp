...
//load strings from resource stringtable and add them to listbox
HWND hList=GetDlgItem(hwnd,IDC_LISTBOX);
const int MAX_STR_LEN=256;
TCHAR txt[MAX_STR_LEN];
for (int i=IDS_STRING1;i<=IDS_STRING8;++i)
  {
  LoadString(GetModuleHandle(0),i,txt,MAX_STR_LEN-1);
  SendMessage(hList,LB_ADDSTRING,0,reinterpret_cast<LPARAM>(txt));
  }
