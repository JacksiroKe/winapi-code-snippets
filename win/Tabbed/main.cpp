...
void CreateControls(HWND,  CREATESTRUCT*);
void AddItemsInListBox(HWND);
void CreateComboBox(HWND, HINSTANCE);
void CreateRadioButtonfirstgroup(HWND, HINSTANCE, int);
void CreateLabels(HWND, HINSTANCE);
void CreateGroups(HWND, HINSTANCE);
void CreateEditBox(HWND, HINSTANCE);
void CreateRadioButtonsecondgroup(HWND, HINSTANCE,int);
void CreateCheckBox(HWND, HINSTANCE,int);
void Createopbutton(HWND, HINSTANCE);
void CreateListBox(HWND, HINSTANCE);
char* RetrieveText(HWND);
void checkbox(HWND, HWND);
void EditText(HWND);
void radiocheck(int,HWND);
void RetrieveTextFromComboBox(HWND);
/**
...
{	
	...
	case WM_CREATE:
		...
		CreateControls(phWnd, reinterpret_cast<CREATESTRUCT*>(lParam));		///<creates all the controls.
		break;
...
void CreateLabels(HWND phWnd, HINSTANCE hInst )
{
	//LABEL FOR THE COMBO BOX
	CreateWindow("Static", "Enter the field of Study:",WS_CHILD|WS_VISIBLE|SS_LEFT,40,80,200,
		20,phWnd,NULL,hInst,NULL);

	//LABEL FOR THE BADIOBUTTONS OF FIRST GROUP
	CreateWindow("Static","Enter the Students Year Of Study:",WS_CHILD|WS_VISIBLE|SS_LEFT,40,160,250									 ,20,phWnd,NULL,hInst,NULL);

	//LABEL FOR THE EDIT BOX
	CreateWindow("Static","Enter the Students Name:",WS_CHILD|WS_VISIBLE|SS_LEFT,510,80,180,20,									phWnd,NULL,hInst,NULL);
	...
	CreateWindow("Static","Enter the Students Hobbies:",WS_CHILD|WS_VISIBLE|SS_LEFT,220,350,280,20,										  phWnd,NULL,hInst,NULL);
}
...
void CreateControls( HWND phWnd, CREATESTRUCT *cs )
{
	//TO CREATE COMBO BOX
	CreateComboBox(phWnd, cs->hInstance);

	//TO CREATE RADIOBUTTON
	CreateRadioButtonfirstgroup(phWnd, cs->hInstance,180);

	//TO CREATE LABELS FOR ALL THE CONTROLS
    CreateLabels(phWnd, cs->hInstance);

	//TO CREATE THE GROUP STRUCTURE
	CreateGroups(phWnd, cs->hInstance);
    
	// CreateEditBox(phWnd);
    CreateEditBox(phWnd, cs->hInstance);	

	//TO CREATE SECOND SET OF RADIOBUTTONS
	CreateRadioButtonsecondgroup(phWnd, cs->hInstance,180);

	//TO CREATE CHECK BOX
	CreateCheckBox(phWnd, cs->hInstance,340);

	//TO CREATE THE SUBMIT AND RESET BUTTONS FOR THE 1ST TAB
	Createopbutton(phWnd, cs->hInstance);
							
	//TO CREATE LIST BOX CONTROL 
	CreateListBox(phWnd, cs->hInstance);
 ...
 * 
 */
void CreateGroups(HWND phWnd, HINSTANCE hInst)
{
...
void CreateRadioButtonfirstgroup(HWND phWnd, HINSTANCE hInst, int y_axis)
{
	...
	for(i=ZERO,x_axis=50;i<THREE;i++,x_axis+=90){
		...
		CreateWindow("Button",radiobuttonnames[i],WS_CHILD|WS_VISIBLE|WS_TABSTOP|BS_AUTORADIOBUTTON,
			...
			x_axis,y_axis,50,100,phWnd,(HMENU)(IDC_RADIO11+i),hInst,NULL);
	}   	
}
...
void CreateRadioButtonsecondgroup(HWND hWnd, HINSTANCE hInst, int y_axis)
{
	...
	for(i=ZERO,x_axis=520;i<THREE;i++,x_axis+=90){
		...
		CreateWindow("Button",radiobuttonnames1[i],WS_CHILD|WS_VISIBLE|WS_TABSTOP|BS_AUTORADIOBUTTON,
			...
			x_axis,y_axis,50,100,hWnd,(HMENU)(IDC_RADIO21+i),hInst,NULL);
	}
}
...
void CreateCheckBox(HWND phWnd, HINSTANCE hInst,int y_axis)
{
	...
	for(i=ZERO,x_axis=50;i<FOUR;i++,x_axis+=150){
		...
		CreateWindow("Button",checkboxnames[i],WS_CHILD|WS_VISIBLE|BS_AUTOCHECKBOX|WS_TABSTOP,
			x_axis,390,100,15,phWnd,(HMENU)(IDC_CHECKBOX1+i),hInst,NULL);
...
void CreateComboBox(HWND phWnd, HINSTANCE hInst)
{
...
void CreateListBox(HWND phWnd, HINSTANCE hInst)
{
	//CREATES THE LIST BOX
	CreateWindow("ListBox",NULL,WS_CHILD|WS_VISIBLE |WS_BORDER |WS_VSCROLL , 200,480,200,50,phWnd,(HMENU)IDC_LISTBOX,hInst,NULL);
}
